...
- [短剧] 内容整理任务 ID 由服务端在入队前直接写入对应 Episode，避免大项目自动保存期间立即刷新丢失生命周期；整理结果和失败状态也会在页面宣告完成前立即持久化，桌面步骤条改为工具栏下方整行展示并收拢长错误文案。
- [短剧] 任一入口触发内容整理后，剧本工具栏下方统一展示可刷新恢复的七步生命周期，依据后台真实任务状态显示创建、排队、原文读取、结构提取、合并校验、项目保存和完成进度。
## v0.0.7
