...
async function runDramaAnalysisTextTask(task: TextTask, origin: string): Promise<TextTaskStep> {
    ...
    if (!analysis) return failTextTask(task, "短剧视觉任务参数不完整", task.attempts || []);
    await scheduleGenerationTask("text", task.id, {
        executionPhase: "submitting",
    ...
    try {
        ...
        if (!response.ok || !payload?.data) return failTextTask(task, payload?.msg || "AI 视觉方案生成失败", task.attempts || []);
        ...
        return failTextTask(task, error instanceof Error ? error.message : "AI 视觉方案生成失败", task.attempts || []);
    }
}
